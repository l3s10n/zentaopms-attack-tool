...
import fun.fireline.exp.zentaopms.*;
...
public class Tools {
    ...
    public static ExploitInterface getExploit(String vulName) {
        ...
        if(vulName.contains("8.2~9.21 sql注入 普通用户")) {
            ei = new ZenTaoPMS_SqlInjection_NormalUser_8to9();
        } else if(vulName.contains("10.x~12.4.3 文件上传 管理员用户")) {
            ei = new ZenTaoPMS_FileUpload_Admin_10to12();
        } else if(vulName.contains("x~11.6 sql注入 普通用户")) {
            ei = new ZenTaoPMS_SqlInjection_NormalUser_xto11();
        } else if(vulName.contains("x~11.6 文件读取 普通用户")) {
            ei = new ZenTaoPMS_FileRead_NormalUser_xto11();
        } else if(vulName.contains("x~11.6 rce 普通用户")) {
            ei = new ZenTaoPMS_RCE_NormalUser_xto11();
        } else if(vulName.contains("16.4~18.0.beta1 sql注入 不需要认证")) {
            ei = new ZenTaoPMS_SqlInjection_NoAuth_16to18();
        } else if(vulName.contains("16.4~18.0.beta1 rce 不需要认证")) {
            ei = new ZenTaoPMS_RCE_NoAuth_16to18();
        }
